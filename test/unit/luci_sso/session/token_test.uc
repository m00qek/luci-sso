...
describe('session.token: create', () => {
	it('returns INVALID_USER_DATA for null', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
	...
	it('uses email as user when present', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			...
			assert.match(
				...
				crypto.jws_verify(injected.native, res.data, SECRET)
			);
		});
	});
...
describe('session.token: verify', () => {
	...
	it('propagates key.get errors', () => {
		mock.inject_all(KEY_FAIL_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW, exp: NOW + 3600 }, SECRET);
	...
	it('returns SESSION_SIGNATURE_INVALID for a token signed with a different secret', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW, exp: NOW + 3600 }, 'zzzzyyyyyxxxxwwwwvvvvuuuuttttssss');
	...
	it('returns MALFORMED_SESSION_TOKEN when exp is missing', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW }, SECRET);
	...
	it('returns MALFORMED_SESSION_TOKEN when exp is not an integer', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW, exp: '1700003600' }, SECRET);
			assert.match(contains({ ok: false, error: 'MALFORMED_SESSION_TOKEN' }), token.verify(deps, tok.data, 0));
		});
	});
	...
	it('returns SESSION_EXPIRED when exp is in the past', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW - 3700, exp: NOW - 100 }, SECRET);
	...
	it('returns SESSION_EXPIRED when exp < now - clock_tolerance', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW - 3700, exp: NOW - 61 }, SECRET);
	...
	it('accepts exp exactly at the clock tolerance boundary (exp == now - tolerance)', () => {
		...
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW - 60, exp: NOW - 60 }, SECRET);
	...
	it('returns MALFORMED_SESSION_TOKEN when iat is missing', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', exp: NOW + 3600 }, SECRET);
			assert.match(contains({ ok: false, error: 'MALFORMED_SESSION_TOKEN' }), token.verify(deps, tok.data, 0));
		});
	});
	...
	it('returns MALFORMED_SESSION_TOKEN when iat is not an integer', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: '1700000000', exp: NOW + 3600 }, SECRET);
			assert.match(contains({ ok: false, error: 'MALFORMED_SESSION_TOKEN' }), token.verify(deps, tok.data, 0));
		});
	});
	...
	it('returns SESSION_NOT_YET_VALID when iat is beyond clock tolerance', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW + 100, exp: NOW + 3700 }, SECRET);
	...
	it('accepts iat within clock tolerance', () => {
		mock.inject_all(KEY_STATE, (injected) => {
			let deps = { fs: injected.fs, clock: injected.clock, native: injected.native, log: () => null };
			let tok = crypto.jws_sign(injected.native, { user: 'x', iat: NOW + 30, exp: NOW + 3630 }, SECRET);
			assert.match(contains({ ok: true }), token.verify(deps, tok.data, 60));
		});
	});
