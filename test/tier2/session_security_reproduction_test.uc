...
it('session: security - verify_session truthiness bypass (Audit B1)', () => {
	...
	factory.with_env({ time: now }, (io) => {
		...
		assert.match(truthy(), !res_exp0.ok, "Session with exp=0 MUST be rejected");
		...
		assert.match(truthy(), !res_no_exp.ok, "Session without exp MUST be rejected");
		...
		assert.match(truthy(), !res_no_iat.ok, "Session without iat MUST be rejected");
