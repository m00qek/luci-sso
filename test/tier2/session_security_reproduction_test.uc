...
describe('session: security', () => {
	it('verify_session truthiness bypass (Audit B1)', () => {
		let now = 1739478000;
		let tolerance = 300;

		mock.inject_all({ fs: {}, native: {} }, (injected) => {
			let deps = { fs: injected.fs, native: injected.native, log: () => null, clock: { time: () => now, sleep: () => null } };
			...
			let token_exp0 = crypto.jws_sign(injected.native, payload_exp0, key).data;
			...
			let token_no_exp = crypto.jws_sign(injected.native, payload_no_exp, key).data;
			...
			let token_no_iat = crypto.jws_sign(injected.native, payload_no_iat, key).data;
