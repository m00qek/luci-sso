...
describe('session: security', () => {
	it('verify_session truthiness bypass (Audit B1)', () => {
		...
		mock.inject_all({ fs: {} }, (injected) => {
			let deps = { fs: injected.fs, native, log: () => null, clock: { time: () => now, sleep: () => null } };
			...
			let token_exp0 = crypto.jws_sign(native, payload_exp0, key).data;
			...
			let token_no_exp = crypto.jws_sign(native, payload_no_exp, key).data;
			...
			let token_no_iat = crypto.jws_sign(native, payload_no_iat, key).data;
