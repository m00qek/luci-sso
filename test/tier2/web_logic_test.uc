...
test('web: security - prevent XSS in redirect location', () => {
	...
	mock.create().spy((io) => {
		...
		assert(index(body, "Redirecting") >= 0, "Should contain redirecting message");
		assert(index(body, malicious_loc) == -1, "Malicious location MUST NOT be present in HTML body");
		assert(index(body, "url=") == -1, "Meta refresh URL MUST NOT be present in body");
	});
});
