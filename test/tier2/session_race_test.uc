...
describe('session: race condition', () => {
	it('should fail instead of falling back to random key when lock is held', () => {
		mock.inject('fs', {
			data: { [KEY_PATH]: "" },
			behavior: {
				mkdir: (path) => {
...
describe('security: secret key', () => {
	it('bootstrap retries and succeeds if file appears (B2)', () => {
		let read_attempts = 0;
	...
	it('bootstrap fails after maximum retries (B2)', () => {
		let read_attempts = 0;
