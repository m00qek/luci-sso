...
it('oidc: discovery - cache robustness & TTL', () => {
	...
	with_context({
		...
		// First call: fetch from network and write to cache
		...
		// Third call with ttl=-1: force refresh regardless of cache age
...
it('oidc: ID token - support multi-audience arrays', () => {
	...
	// 1. Success: Correct ID in array (RFC 7519: azp mandatory when aud is array)
