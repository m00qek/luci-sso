...
it('native: JWK - EC P-256 to PEM conversion', () => {
    ...
    assert.match(truthy(), pem, "Should successfully convert EC JWK to PEM");
    assert.match(truthy(), index(pem, "BEGIN PUBLIC KEY") >= 0, "Should produce a PUBLIC KEY PEM");
    ...
    assert.match(truthy(), res, "Produced PEM must work for ES256 verification");
});
