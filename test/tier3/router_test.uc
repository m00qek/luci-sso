...
it('router: callback - reject state replay', () => {
	with_context({
		...
		// First attempt — consumes the state
		router.handle(deps, MOCK_CONFIG, req, TEST_POLICY);

		// Second attempt — state already consumed
		let res = router.handle(deps, MOCK_CONFIG, req, TEST_POLICY);
		assert.match(falsy(), res.ok);
...
it('router: security - skip token registration on verification failure', () => {
	with_context({
		...
		// Second attempt: same access_token. If NOT registered: 401. If was registered: 403 TOKEN_REPLAYED.
