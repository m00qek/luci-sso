...
describe('encoding: url', () => {
	it('normalize_url removes trailing slashes', () => {
		assert.match("https://idp.com", encoding.normalize_url("https://idp.com/").data);
		assert.match("https://idp.com", encoding.normalize_url("https://idp.com///").data);
	});

	it('normalize_url case normalization', () => {
		assert.match("https://idp.com", encoding.normalize_url("HTTPS://IDP.COM").data);
	});

	it('normalize_url preservation of path', () => {
		assert.match("https://idp.com/auth", encoding.normalize_url("https://idp.com/auth/").data);
	});

	it('normalize_url handles non-string safely', () => {
