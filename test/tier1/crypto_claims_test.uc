...
it('mandatory claims enforced (B2)', () => {
    ...
    let res = crypto.jwt_verify(PLUMBING_RSA.token, PLUMBING_RSA.pubkey, opts);
    assert.match("MISSING_EXP_CLAIM", res.error, "Should fail on missing exp");

    // 2. W3 Fix: even with allow_missing_claims, it should still FAIL
    let opts_allow = { ...opts, allow_missing_claims: true };
    let res_allow = crypto.jwt_verify(PLUMBING_RSA.token, PLUMBING_RSA.pubkey, opts_allow);
    assert.match("MISSING_EXP_CLAIM", res_allow.error, "W3: allow_missing_claims MUST be ignored/removed");
});

it('mandatory iat enforced', () => {
