...
it('encoding: security - safe_json does not leak raw fragments on failure', () => {
    ...
    assert.match(truthy(), !res.ok, "Should fail to parse malformed JSON");
    assert.match(truthy(), res.raw_fragment === undefined, "Error response MUST NOT contain raw_fragment (W4)");
});
...
it('encoding: security - safe_json handles binary input safely', () => {
    ...
    assert.match(truthy(), !res.ok, "Binary data is not valid JSON");
    assert.match(truthy(), res.raw_fragment === undefined, "Should not leak binary fragments");
});
